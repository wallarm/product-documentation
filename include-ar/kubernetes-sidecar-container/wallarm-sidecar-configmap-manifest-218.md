...
data:
  default: |
    geo $remote_addr $wallarm_mode_real {
      # لو سمحت استبدل <WALLARM_MODE> فيما يلي بوضع فلترة الطلبات:
      # off لتعطيل معالجة الطلبات
      # monitoring لمعالجة الطلبات دون حجبها
      # block لمعالجة كل الطلبات وحجب تلك الضارة
      default <WALLARM_MODE>;
      # عناوين الـ IP والقواعد لماسحات السحابة الأمريكية
      ...
      # عناوين الـ IP والقواعد لماسحات السحابة الأوروبية
    ...
    server {
        ...
        # wallarm_instance 1;
        # افتح التعليق على السطر التالي لتفعيل
        # وظيفة حجب الـ IP
        # wallarm_acl default;
        set_real_ip_from 0.0.0.0/0;
        real_ip_header X-Forwarded-For;
        location / {
                # لو سمحت استبدل <APP_CONTAINER_PORT> فيما يلي برقم البورت
                # الذي يستقبل به الحاوية الطلبات الواردة،
                # القيمة يجب أن تطابق ports.containerPort
