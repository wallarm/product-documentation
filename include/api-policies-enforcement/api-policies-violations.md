...
* **Requesting endpoint with invalid parameter value** - a request parameter's value is not in correspondence with its type/format defined by your specification
...
The system can perform the following actions when an inconsistency is found:
...
Note that several specifications can be used for setting policies. When one request falls on two different specifications (the same policy and different actions in different specifications), the following will happen:
