...
```
sudo sh ./wallarm-7.1.3.x86_64-glibc.sh -- -h
```
...
=== "US Cloud"
    ...
    ```bash
    sudo env WALLARM_LABELS='group=<GROUP>' sh wallarm-7.1.3.x86_64-glibc.sh -- --batch -t <TOKEN> -c US
    ```

    If using the ARM64 version:

    ```bash
    sudo env WALLARM_LABELS='group=<GROUP>' sh wallarm-7.1.3.aarch64-glibc.sh -- --batch -t <TOKEN> -c US
    ```
=== "EU Cloud"
    If using the x86_64 version:

    ```bash
    sudo env WALLARM_LABELS='group=<GROUP>' sh wallarm-7.1.3.x86_64-glibc.sh -- --batch -t <TOKEN>
    ```

    If using the ARM64 version:

    ```bash
    sudo env WALLARM_LABELS='group=<GROUP>' sh wallarm-7.1.3.aarch64-glibc.sh -- --batch -t <TOKEN>
    ```
...
=== "US Cloud"
    ...
    sudo sh wallarm-7.1.3.x86_64-glibc.sh -- --batch --install-only
    ...
    sudo sh wallarm-7.1.3.aarch64-glibc.sh -- --batch --install-only
    sudo env WALLARM_LABELS='group=<GROUP>' /opt/wallarm/setup.sh --batch --register-only -t <TOKEN> -c US
    ```
=== "EU Cloud"
    If using the x86_64 version:
