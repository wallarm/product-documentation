...
```
sudo sh ./wallarm-6.7.1.x86_64-glibc.sh -- -h
```
...
=== "US Cloud"
    ...
    sudo env WALLARM_LABELS='group=<GROUP>' sh wallarm-6.7.1.x86_64-glibc.sh -- --batch -t <TOKEN> -c US
...
=== "EU Cloud"
    ```bash
    # If using the x86_64 version:
    sudo env WALLARM_LABELS='group=<GROUP>' sh wallarm-6.7.1.x86_64-glibc.sh -- --batch -t <TOKEN>
...
=== "US Cloud"
    ...
    sudo sh wallarm-6.7.1.x86_64-glibc.sh -- --batch --install-only
