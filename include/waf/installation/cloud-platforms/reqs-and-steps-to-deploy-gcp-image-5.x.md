...
* Access to `https://us1.api.wallarm.com:444` for working with US Wallarm Cloud, `https://api.wallarm.com:444` for working with EU Wallarm Cloud, or `https://me1.api.wallarm.com:444` for working with ME Wallarm Cloud. If access can be configured only via the proxy server, then use the [instructions][wallarm-api-via-proxy]
