...
* Access to `https://us1.api.wallarm.com` for working with US Wallarm Cloud or `https://api.wallarm.com` for working with EU Wallarm Cloud. If access can be configured only via the proxy server, then use the [instructions][wallarm-api-via-proxy]
...
* To launch the instance with the filtering node version 7.x, please use the following image name:
    ...
    ```bash
    wallarm-node-195710/wallarm-node-7-1-3-20260827-223907
    ```
...
2. Execute the command [`gcloud compute images list`](https://cloud.google.com/sdk/gcloud/reference/compute/images/list) with the following parameters:
    ...
    ```bash
    gcloud compute images list --project wallarm-node-195710 --filter="name~'wallarm-node-7-1-*'" --no-standard-images
    ```
