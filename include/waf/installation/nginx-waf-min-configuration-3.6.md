...
* `/etc/nginx/conf.d/wallarm-status.conf` with Wallarm node monitoring settings. Detailed description is available at the [link][wallarm-status-instr]
...
By default, the filtering node is in the status `off` and does not analyze incoming requests. To enable request analysis, follow these steps:
...
When operating in the `monitoring` mode, the filtering node searches for attack signs in requests but does not block detected attacks. We recommend keeping the traffic flowing via the filtering node in the `monitoring` mode for several days after the filtering node deployment and only then enable the `block` mode. [Learn recommendations on the filtering node operation mode setup →][waf-mode-recommendations]
...
The Wallarm node uses the in-memory storage Tarantool. Learn more about the amount of required resources [here][memory-instr]. Note that for testing environments you can allocate lower resources than for the production ones.
