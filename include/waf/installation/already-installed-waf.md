!!! info "If Wallarm WAF is already installed in your environment"
    ...
    * If the version `2.16.x` is installed, then follow the instruction for the [WAF node 2.16][2.16-installation-instr] and for [separate postanalytics 2.16][2.16-install-postanalytics-instr].
    * If the version `2.14.x` is installed, then follow the current instruction for the WAF node and for [separate postanalytics 2.14][2.14-install-postanalytics-instr] or update [WAF node packages][nginx-modules-update-docs] and [separate postanalytics packages][separate-postanalytics-update-docs] to 2.16 in all installations.
