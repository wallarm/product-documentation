...
NGINX Plus ve bağımlılıklarını yüklemek için [resmi NGINX talimatlarını](https://www.nginx.com/resources/admin-guide/installing-nginx-plus/) kullanın.

!!! info "Amazon Linux 2.0.2021x ve daha eski sürümlerde yükleme"
    NGINX Plus'ı Amazon Linux 2.0.2021x ve daha eski sürümde yüklemek için CentOS 7 talimatlarını kullanın.

## 2. Wallarm Depolarını Ekleyin

Wallarm düğümü, Wallarm depolarından yüklenir ve güncellenir. Depoları eklemek için platformunuz için komutları kullanın:
...
=== "CentOS 7.x"
    ...
    ```
=== "Amazon Linux 2.0.2021x ve daha eski"
    ```bash
...
=== "Ubuntu"
    ...
    sudo apt -y install --no-install-recommends wallarm-node nginx-plus-module-wallarm
    ```
=== "CentOS veya Amazon Linux 2.0.2021x ve daha eski"
    ```bash
...
2. `worker_processes` yönergesinin hemen ardından aşağıdaki yönergeyi ekleyin:
    ...
    Eklenen yönerge ile birlikte yapılandırma örneği:
