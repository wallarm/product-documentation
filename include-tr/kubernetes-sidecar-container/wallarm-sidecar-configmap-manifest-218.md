...
data:
  default: |
    geo $remote_addr $wallarm_mode_real {
      # Lütfen aşağıdaki <WALLARM_MODE>'i istek filtreleme modu ile değiştirin:
    ...
    server {
        ...
        # wallarm_instance 1;
        # Aşağıdaki satırın yorumunu kaldırarak
        # IP engelleme işlevselliğini etkinleştirin
        # wallarm_acl default;
        set_real_ip_from 0.0.0.0/0;
        real_ip_header X-Forwarded-For;
        location / {
                # Lütfen aşağıdaki <APP_CONTAINER_PORT>'ı, container'ın gelen istekleri kabul ettiği port numarası ile değiştirin,
                # değer, ana uygulama container'ınızın tanımındaki ports.containerPort ile aynı olmalıdır
