...
spec:
  ...
  template:
    ...
    spec:
      containers:
        # Wallarm elementi: Wallarm sidecar konteyner tanımı
        - name: wallarm
          ...
          # Dağıtma rolüne sahip kullanıcının kullanıcı adı
          - name: DEPLOY_USER
            value: "username"
          # Dağıtma rolüne sahip kullanıcının şifresi
        ...
        - name: myapp
          ...
          - containerPort: 8080
      volumes:
      # Wallarm elementi: wallarm-nginx-conf hacminin tanımı
      - name: wallarm-nginx-conf
        configMap:
