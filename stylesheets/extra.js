// Copy page markdown to clipboard — fetches the .md companion (see scripts/generate_raw_markdown.py)
...
// Page-actions dropdown: caret toggle, outside-click/Escape close, copy-to-clipboard items
(function () {
  ...
  // Close the dropdown on scroll (capture phase catches any scrollable ancestor)
...
// Mobile nav fix: reset the drawer's expanded toggles to the active page's path on open
...
/* ============================================================
 * Navigation tabs — inject prev/next scroll-arrows when the row overflows
 * ============================================================ */
(function () {
  var CHEV =
  ...
  function initTabScroll() {
    ...
    if (!list.dataset.tabScrollWired) {
      ...
      // Center the active tab instantly on load (smooth scroll would flicker the arrows)
...
(function () {
  var SEARCH_CSS = [
    /* Square off the whole widget (!important beats Zensical's own radius rules) */
    '*, *::before, *::after {',
  ...
  // Insert our <style> once the shadow root holds the search input
  ...
  // Watch the shadow root directly and re-inject on rebuilds (inject() dedupes)
  ...
  // Catch the shadow host (a <div> appended to <body>) as the light DOM settles
  function scan() {
