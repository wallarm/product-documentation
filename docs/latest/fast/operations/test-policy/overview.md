...
* The way tests are performed
    ...
    * vulnerability detection using built-in FAST extensions, also known as *detects*
    * vulnerability detection using custom extensions
...
* Elements of the baseline request that FAST node processes during application testing
    ...
    Points allowed for processing are configured in the **Insertion points** > **Where in the request to include** section of the policy editor on your Wallarm account. See details about insertion points by this [link][doc-insertion-points].

* Elements of the baseline request that FAST node does not process during application testing

    Points that are not allowed for processing are configured in the **Insertion points** > **Where in the request to exclude** section of the test policy settings on your Wallarm account. You can find more details about insertion points via this [link][doc-insertion-points].
...
!!! warning "Policy scope"
    When explicitly excluding points, the FAST node processes only the points allowed by the policy.
...
??? info "Policy example"
    ...
    The image above demonstrates the policy used by the FAST node in vulnerability detection. This policy allows processing of all GET parameters in the baseline request excluding the `token` GET parameter, which is always passed to the target application untouched.
