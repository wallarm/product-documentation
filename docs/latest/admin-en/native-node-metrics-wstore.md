...
[helm-protocol]:        ../installation/native-node/helm-chart-conf.md#configaggregationmetricsprotocol
[helm-yaml]:            ../installation/native-node/helm-chart-conf.md
...
* The [Postanalytics module][postanalytics-module] uses **wstore** for local traffic processing. Its metrics are prefixed with `wallarm_wstore_*` and reflect the performance of the Postanalytics module. 
    ...
    The available metric groups are listed [below](#blocked-requests-metrics). The exact list of metrics may vary depending on the Native Node version. Changes are reflected in the [Native Node changelog][native-node-changelog].
...
**For [Helm chart][helm-chart]:**

Configure the following settings in `values.yaml` [during or after Native Node deployment][helm-yaml]:

* [`config.aggregation.metrics.listenAddress`][helm-listenAddress] — specifies the host and port
* [`config.aggregation.metrics.protocol`][helm-protocol] — specifies the protocol
