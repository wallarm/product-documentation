...
!!! info "Self-hosted nodes only"
    This article describes how TLS termination is handled for **self-hosted** Wallarm nodes ([NGINX Node][nginx-node] and [Native Node][native-node]), including certificate requirements and management. It does not cover deployments using [Security Edge][security-edge].
