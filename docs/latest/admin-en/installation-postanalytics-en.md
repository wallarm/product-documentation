...
* Access to `https://api.wallarm.com:443` if working with EU Wallarm Cloud or to `https://us1.api.wallarm.com:443` if working with US Wallarm Cloud. If access can be configured only via the proxy server, then use the [instructions][configure-proxy-balancer-instr]
...
!!! warning "Protect installed postanalytics module"
    ...
    *   Allow the HTTPS traffic to and from the Wallarm API servers, so the postanalytics module can interact with these servers:
        *   `api.wallarm.com:443` is the API server in the EU Wallarm Cloud
        *   `us1.api.wallarm.com:443` is the API server in the US Wallarm Cloud
