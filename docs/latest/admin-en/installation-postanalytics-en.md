...
=== "API token"
    ...
    sudo env WALLARM_LABELS='group=<GROUP>' sh wallarm-6.0.0.x86_64-glibc.sh postanalytics

    # If using the ARM64 version:
    sudo env WALLARM_LABELS='group=<GROUP>' sh wallarm-6.0.0.aarch64-glibc.sh postanalytics
    ```        
...
=== "Node token"
    ```bash
    # If using the x86_64 version:
    sudo sh wallarm-6.0.0.x86_64-glibc.sh postanalytics

    # If using the ARM64 version:
    sudo sh wallarm-6.0.0.aarch64-glibc.sh postanalytics
    ```
...
1. When launching the installation script for the NGINX-Wallarm module on a separate server, include the `filtering` option, for example:
    ...
    === "API token"
        ```bash
        # If using the x86_64 version:
        sudo env WALLARM_LABELS='group=<GROUP>' sh wallarm-6.0.0.x86_64-glibc.sh filtering

        # If using the ARM64 version:
        sudo env WALLARM_LABELS='group=<GROUP>' sh wallarm-6.0.0.aarch64-glibc.sh filtering
        ```        
    ...
    === "Node token"
        ```bash
        # If using the x86_64 version:
        sudo sh wallarm-6.0.0.x86_64-glibc.sh filtering

        # If using the ARM64 version:
        sudo sh wallarm-6.0.0.aarch64-glibc.sh filtering
        ```
