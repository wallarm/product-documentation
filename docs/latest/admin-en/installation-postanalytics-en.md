...
=== "x86_64 version"
    ```bash
    curl -O https://meganode.wallarm.com/7.1/wallarm-7.1.1.x86_64-glibc.sh
    ```
=== "ARM64 version"
    ```bash
    curl -O https://meganode.wallarm.com/7.1/wallarm-7.1.1.aarch64-glibc.sh
    ```
...
=== "API token"
    ...
    ```bash
    sudo env WALLARM_LABELS='group=<GROUP>' sh wallarm-7.1.1.x86_64-glibc.sh postanalytics
    ```

    If using the ARM64 version:

    ```bash
    sudo env WALLARM_LABELS='group=<GROUP>' sh wallarm-7.1.1.aarch64-glibc.sh postanalytics
    ```        
...
=== "Node token"
    ...
    ```bash
    sudo sh wallarm-7.1.1.x86_64-glibc.sh postanalytics
    ```

    If using the ARM64 version:

    ```bash
    sudo sh wallarm-7.1.1.aarch64-glibc.sh postanalytics
    ```
...
1. When launching the installation script for the NGINX-Wallarm module on a separate server, include the `filtering` option, for example:
    ...
    === "API token"
        ...
        ```bash
        sudo env WALLARM_LABELS='group=<GROUP>' sh wallarm-7.1.1.x86_64-glibc.sh filtering
        ```

        If using the ARM64 version:

        ```bash
        sudo env WALLARM_LABELS='group=<GROUP>' sh wallarm-7.1.1.aarch64-glibc.sh filtering
        ```        
    ...
    === "Node token"
        ...
        ```bash
        sudo sh wallarm-7.1.1.x86_64-glibc.sh filtering
        ```

        If using the ARM64 version:

        ```bash
        sudo sh wallarm-7.1.1.aarch64-glibc.sh filtering
        ```
