...
| `wallarm_attack_point_list` (NGINX node 5.2.0 and above) | String | Lists [request points](../user-guides/rules/request-processing.md) containing malicious payloads. Multiple points containing malicious payloads are concatenated using `|`. If several parsers identify a point, these are listed without a divider.<br>Example: `[get_name]|[uri][percent]` if a malicious payload is detected in a query parameter.<br>Note that this log data may differ from the simplified, user‑friendly view presented in the Wallarm Console UI. |
| `wallarm_attack_stamp_list` (NGINX node 5.2.0 and above) | String | Lists internal Wallarm IDs for each attack sign detected in a request. Multiple sign IDs are concatenated using `|`. If the same attack sign is detected at multiple parsing stages, the IDs may repeat. For example, a SQLi attack like `union+select+1` could return `7|7`, showing multiple detections.<br>Note that this log data may differ from the simplified, user‑friendly view presented in the Wallarm Console UI. |
