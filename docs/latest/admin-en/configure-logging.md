...
|`wallarm_request_cpu_time`|Float|Actual time in seconds the CPU spent processing the request.|
|`wallarm_request_mono_time`|Float|Actual time in seconds the CPU spent processing the request + waiting time. For example, if the request has come, was waiting for 3 seconds and processed by CPU for 1 second, then: <ul><li>`wallarm_request_cpu_time` = 1s</li><li>`wallarm_request_mono_time` = 4s</li></ul>|
