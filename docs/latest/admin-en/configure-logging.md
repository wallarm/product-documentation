...
* `wcli-out.log`: logs of most Wallarm services, including brute force detection, attack export to the Cloud, and the status of node synchronization with the Cloud, API Discovery etc.
* `wd.log`: logs of the `wd` (Wallarm Daemon) service — process management (service startups, status changes, warnings) and the node's registration and Wallarm Cloud synchronization jobs (data sync, IP lists, IP feeds, API specifications).
...
2.  Enable the extended logging format by adding the following directive into the same block as in the first step:
    ...
    !!! info "Conditional Logging"
        With the directive listed above, all processed requests will be logged to a log file, including those that are not related to an attack.
        ...
        This may be useful if you want to reduce the log file size, or if you integrate a filter node with one of the [SIEM solutions](https://www.wallarm.com/what/siem-whats-security-information-and-event-management-technology-part-1).          
