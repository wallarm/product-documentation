...
| `wallarm_attack_point_list` (NGINX node 5.2.0 and above) | String | Lists [parsers](../user-guides/rules/request-processing.md) the Node used to identify request's parameters containing malicious payloads. Multiple parsers are concatenated using `|`.<br>Example: `[get_name]|[uri][percent]` if a malicious payload is detected in a query parameter. |
| `wallarm_attack_stamp_list` (NGINX node 5.2.0 and above) | String | Lists internal Wallarm IDs for each [attack sign](../about-wallarm/protecting-against-attacks.md#malicious-payload) detected in a request. Multiple sign IDs are concatenated using `|`. |
