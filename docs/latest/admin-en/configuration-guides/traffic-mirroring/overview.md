...
Traffic mirroring enables original incoming traffic to be sent to multiple backends in parallel. Installing a Wallarm node as the additional backend lets you run filtration of traffic mirror (copy) with no impact on the clients - any incoming requests will reach the servers they are addressed to.

Below is an example of the traffic flow diagram with the mirroring option enabled:
...
Despite the safety of this deployment approach, it has some limitations:
...
Traffic mirroring is supported by many web servers. In the following links, you will find the **example configuration** for the most popular of them:
