...
To complete the steps, you need accounts with administration rights both for Wallarm and G Suite.
...
* If you have [multiple tenants](../../../installation/multi-tenant/overview.md), with G Suite, you can use the [tenant dependent permissions](intro.md#tenant-dependent-permissions) option, make a decision on that together with Wallarm support.
...
1. In the SSO configuration wizard, at the **Send details** step, review the Wallarm metadata that should be passed to the G Suite service.
    ...
    * **Assertion Consumer Service URL (ACS URL)** is the address on the Wallarm side of the application on which the identity provider sends requests with the SamlResponse parameter.

1. Copy metadata or save them as XML. 

## Step 3 (G Suite): Configure application

To configure the application in G Suite:
...
1. Enter the Wallarm metadata. Required fields:
...
1. Provide G Suite users with access to the created application via **Edit Service** → **Service status** → **ON for everyone**.
1. Save the changes.
