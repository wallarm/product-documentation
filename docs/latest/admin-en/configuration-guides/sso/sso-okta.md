...
To complete the steps, you need accounts with administration rights both for Wallarm and Okta.
...
* If you have [multiple tenants](../../../installation/multi-tenant/overview.md), with Okta, you can use the [tenant dependent permissions](intro.md#tenant-dependent-permissions) option, make a decision on that together with Wallarm support.
...
1. In the SSO configuration wizard, at the **Send details** step, review the Wallarm metadata that should be passed to the Okta service.
...
1. Proceed and enter the Wallarm metadata. Required fields:
