...
Wallarm filtering node performs many functions, each requiring CPU resources. To reach the right balance between functionality and consumed resources, follow the [best practices](../../faq/cpu.md) and carefully consider which node functions are required in your case and which can be turned off.
