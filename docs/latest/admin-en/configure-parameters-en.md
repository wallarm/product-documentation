...
When enabled, the Node sends stream and message data to wstore, allowing [API Discovery](../api-discovery/overview.md) to build the API inventory and display gRPC/WebSocket endpoints in the Wallarm Console UI.

This directive does not affect filtering, detection logic, or the stream/message metrics generated by the Node.

The directive is only applicable if [`wallarm_parse_websocket`](#wallarm_parse_websocket) is set to `on`.

Available starting from release 6.4.0.
...
!!! warning "Rename `wallarm_tarantool_upstream` to `wallarm_wstore_upstream`"
    In NGINX Node version 6.x and later, this parameter has been [renamed](https://docs.wallarm.com/updating-migrating/what-is-new/#replacing-tarantool-with-wstore-for-postanalytics) to [`wallarm_wstore_upstream`](#wallarm_wstore_upstream), with no changes to its logic.
