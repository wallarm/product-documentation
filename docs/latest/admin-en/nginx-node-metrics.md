...
!!! warning "Keep the `wd` scrape targets in sync"
    Ports `9001` and `9003`, `9010` are scraped by `wd` at fixed endpoints. If you change a service's metrics port, you also need to update the endpoint `wd` scrapes it from — otherwise `wd` stops collecting that service's metrics and it disappears from the aggregated `9445` endpoint:
