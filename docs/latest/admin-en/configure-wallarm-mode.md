...
=== "Docker NGINX‑based image"
    ...
    ```
    docker run -d -e WALLARM_API_TOKEN='XXXXXXX' -e WALLARM_LABELS='group=<GROUP>' -e NGINX_BACKEND='example.com' -e WALLARM_API_HOST='us1.api.wallarm.com' -e WALLARM_MODE='monitoring' -p 80:80 wallarm/node:5.0.3-1
    ```
...
=== "Connectors"
    ...
    For [Security Edge connectors](../installation/connectors/overview.md#wallarm-edge-connectors), you specify the `wallarm_mode` value in the **Filtration mode** selector during the connector deployment.
