...
!!! warning "The upgraded version of Community Ingress NGINX Controller"
    If you upgrade the node from version 3.4 or lower, please note that the version of Community Ingress NGINX Controller the Wallarm Ingress controller is based on has been upgraded from 0.26.2 to 1.8.0.
    
    Since the operation of Community Ingress NGINX Controller 1.8.0 has been significantly changed, its configuration has to be adjusted to these changes during the Wallarm Ingress controller upgrade.
...
2. Run the plugin:
    ...
    ```bash
    helm diff upgrade <RELEASE_NAME> -n <NAMESPACE> wallarm/wallarm-ingress --version 4.6.4 -f <PATH_TO_VALUES>
    ```
...
2. Deploy the Ingress controller 4.6:
    ...
    ```bash
    helm install <RELEASE_NAME> -n <NAMESPACE> wallarm/wallarm-ingress --version 4.6.4 -f <PATH_TO_VALUES>
    ```
...
2. Delete listed objects and re‑create the relese:
    ...
    helm upgrade <RELEASE_NAME> -n <NAMESPACE> wallarm/wallarm-ingress --version 4.6.4 -f `<PATH_TO_VALUES>`
    ```
...
1. Check that the version of the Helm chart was updated:
    ...
    The chart version should correspond to `wallarm-ingress-4.6.4`.
