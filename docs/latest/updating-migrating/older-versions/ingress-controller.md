...
2. Run the plugin:
    ...
    ```bash
    helm diff upgrade <RELEASE_NAME> -n <NAMESPACE> wallarm/wallarm-ingress --version 4.4.3 -f <PATH_TO_VALUES>
    ```
...
2. Deploy the Ingress controller 4.4:
    ...
    ```bash
    helm install <RELEASE_NAME> -n <NAMESPACE> wallarm/wallarm-ingress --version 4.4.3 -f <PATH_TO_VALUES>
    ```
...
2. Delete listed objects and re‑create the relese:
    ...
    helm upgrade <RELEASE_NAME> -n <NAMESPACE> wallarm/wallarm-ingress --version 4.4.3 -f `<PATH_TO_VALUES>`
    ```
...
1. Check that the version of the Helm chart was updated:
    ...
    The chart version should correspond to `wallarm-ingress-4.4.3`.
