...
2. Run the plugin:
    ...
    ```bash
    helm diff upgrade <RELEASE_NAME> -n <NAMESPACE> wallarm/wallarm-sidecar --version 4.10.4 -f <PATH_TO_VALUES>
    ```
...
``` bash
helm upgrade <RELEASE_NAME> -n <NAMESPACE> wallarm/wallarm-sidecar --version 4.10.4 -f <PATH_TO_VALUES>
```
