...
2. Assign an application the unique ID in the node configuration via:
    ...
    * The [Ingress annotation](../../admin-en/configure-kubernetes-en.md#supported-wallarm-ingress-annotations) `wallarm-application` if Wallarm is installed as the Ingress controller.
    * For Native Node all-in-one installer, Docker image and AWS AMI, use the [`route_config.wallarm_application`](../../installation/native-node/all-in-one-conf.md#route_configwallarm_application) parameter.
...
If the application is properly configured, its name will be displayed in the details of attacks aimed at this application. To test the application configuration, you can send the [test attack](../../admin-en/uat-checklist-en.md#node-registers-attacks) to the application address.
