...
Files are saved in JSON or New Line Delimited JSON (NDJSON) format using the naming convention `hitlogs-{timestamp}.json` or `hitlogs-{timestamp}.jsonl` correspondingly.
