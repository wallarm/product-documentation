...
* **Local settings (default)**: this mode uses settings from a filter node configuration file.
* **Safe blocking**: all malicious requests originating from [graylisted IPs](../ip-lists/overview.md) are blocked.
...
[Administrators](users.md#user-roles) can set up logout timeouts for the company account. Settings will affect all account users. Idle and absolute timeouts can be set.