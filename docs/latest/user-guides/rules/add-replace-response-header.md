...
The **Change server response headers** [rule](../../user-guides/rules/rules.md) allows adding and deleting server response headers and changing their values.

This rule type is most often used to configure an additional layer of application security, for example:

* To add the response header [`Content-Security-Policy`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy) controlling the resources the client is allowed to load for a given page. This helps guard against [XSS](../../attacks-vulns-list.md#crosssite-scripting-xss) attacks.
...
* To change the NGINX header `Server` or any other header containing the data on installed module versions. This data can be potentially used by an attacker to discover vulnerabilities of installed module versions and as a result, to exploit discovered vulnerabilities.
