...
* Node IP address
* Origin IP associated with the protected domain
...
## Updating the origin IP address of the protected domain

If your hosting provider dynamically updates the IP address associated with the protected domain, please keep the origin IP address specified in the CDN node configuration up to date. Otherwise, the CDN node will try to proxy requests to an incorrect IP address that may cause the node service downtime.

To update the origin IP, use the **Edit origin IP address** option.
