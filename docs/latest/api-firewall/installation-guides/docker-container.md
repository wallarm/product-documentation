...
| `APIFW_SERVER_ROOT_CA`<br>(only if the `APIFW_SERVER_INSECURE_CONNECTION` value is `false`) | The path to the protected application server's CA certificate in the Docker container. The CA certificate must be mounted to the API Firewall Docker container first. | No |
