...
* For requests to be sent from the **own client**, it is required to pass the Global Administrator's [API token](../../user-guides/settings/api-tokens.md#configuring-tokens) in the request.
...
1. Send the POST request to the route `/v1/objects/client/create` with the following parameters:
    ...
    `X-WallarmApi-Token` | Appropriate [API token](../../user-guides/settings/api-tokens.md#configuring-tokens) of the Global administrator user. | Header | Yes, when sending a request from your own client
