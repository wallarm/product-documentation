...
If you deployed the [Native Node AMI](../../../installation/native-node/aws-ami.md) for use with connectors or TCP traffic mirror, traffic routing is configured as part of the connector setup — see the respective [connector guide](../../../installation/connectors/overview.md#supported-platforms).
...
2. **Restrict direct access to the application** — use security groups, network ACLs, or service-level policies to ensure the application **only** accepts traffic from the Wallarm Node.
3. **Validate** — confirm that requests bypassing the Node are rejected.
...
4. **Lock down the application security group.** The application instances should **only** accept traffic from the Wallarm Node's security group:
...
5. **Verify.** Attempt to access the application directly by its IP or DNS name, bypassing the ALB. The connection should be refused.
...
4. **Lock down the application security group** to accept traffic only from the Wallarm Node:
...
5. **Use network ACLs as an additional layer.** Restrict the application subnet's network ACL to only accept traffic from the Wallarm Node subnet:

    ```
    Application Subnet NACL - Inbound Rules:
    ┌──────┬──────────┬──────────┬──────────────────────────┐
...
2. **Add a custom origin header.** Configure CloudFront to pass a secret header to the origin (e.g., `X-CloudFront-Secret: <value>`). This serves as a shared secret to verify that traffic comes from CloudFront.
...
3. **Validate the header on the Wallarm Node.** Configure the Wallarm Node (via NGINX configuration) to reject requests that do not include the correct header:
...
4. **Restrict the Wallarm Node security group to CloudFront IPs.** AWS publishes the [CloudFront IP ranges](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/LocationsOfEdgeServers.html). Use an [AWS-managed prefix list](https://docs.aws.amazon.com/vpc/latest/userguide/managed-prefix-lists-referencing.html) for CloudFront (`com.amazonaws.global.cloudfront.origin-facing`) in the security group:

    ```
    Wallarm Node Security Group - Inbound Rules:
    ┌──────────┬──────────┬────────────────────────────────────────────────┐
    │ Protocol │ Port     │ Source                                         │
    ├──────────┼──────────┼────────────────────────────────────────────────┤
...
5. **Lock down the application** to only accept traffic from the Wallarm Node, as described in the [ALB section above](#application-load-balancer-alb).
...
3. **Lock down the application security group** to only accept traffic from the Wallarm Node:
...
4. **Use API Gateway resource policies (REST API).** For REST APIs, add a [resource policy](https://docs.aws.amazon.com/apigateway/latest/developerguide/apigateway-resource-policies.html) that restricts invocations to known source VPCs or IP ranges if applicable.

5. **Enable mutual TLS (optional).** For an additional layer of authentication between API Gateway and the Wallarm Node, configure [mutual TLS](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-mutual-tls.html) on the API Gateway.
