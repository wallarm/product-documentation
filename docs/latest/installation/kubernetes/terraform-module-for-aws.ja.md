...
Wallarmは、Terraform互換環境から[AWS](https://aws.amazon.com/)にノードをデプロイするための[Terraformモジュール](https://registry.terraform.io/modules/wallarm/wallarm/aws/)を提供します。これらの指示を使用してモジュールを探索し、提供されるデプロイメント例を試してください。

WallarmのTerraformモジュールを実装することで、2つの主要なWallarmデプロイメントオプション：**プロキシ**と**ミラー**のセキュリティソリューションに対応した解決策を提供しました。デプロイメントオプションは`preset` Wallarmモジュール変数により簡単に制御できます。
