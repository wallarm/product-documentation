...
`WALLARM_WSTORE__SERVICE__PROTOCOL` (0.19.0 and higher) | Specifies the protocol family that **wstore** uses for incoming connections. Possible values: tcp - dual-stack mode (listens on both IPv4 and IPv6); tcp4 - IPv4 only; tcp6 - IPv6 only. The default value is `"tcp4"`. | No
