...
```
docker pull wallarm/node-native-aio:0.25.4
```
...
=== "US Cloud"
    ```bash
    docker run -d -e WALLARM_API_TOKEN='XXXXXXX' -e WALLARM_LABELS='group=<GROUP>' -e WALLARM_API_HOST='us1.api.wallarm.com' -v ./wallarm-node-conf.yaml:/opt/wallarm/etc/wallarm/go-node.yaml -p 80:5050 wallarm/node-native-aio:0.25.4
    ```
=== "EU Cloud"
    ```bash
    docker run -d -e WALLARM_API_TOKEN='XXXXXXX' -e WALLARM_LABELS='group=<GROUP>' -e WALLARM_API_HOST='api.wallarm.com' -v ./wallarm-node-conf.yaml:/opt/wallarm/etc/wallarm/go-node.yaml -p 80:5050 wallarm/node-native-aio:0.25.4
    ```
...
[`WALLARM_WSTORE__METRICS__LISTEN_ADDRESS`][native-node-metrics-wstore] (0.20.0 and higher) | Specifies the host and port on which **wstore** exposes Prometheus metrics.<br>By default: `"127.0.0.1:9001"`. | No
