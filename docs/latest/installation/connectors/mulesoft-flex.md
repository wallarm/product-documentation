...
| `real_host_header` (policy version 1.2.0+) | Specifies which HTTP header to use as the original request host when behind a proxy or load balancer. Default: `Host`. | Yes |
...
| `verbose_logging` (policy version 1.3.0+) | Enables extended debug logging. When set to `true`, the connector logs full request and response details sent to the Wallarm Node, including final headers and bodies. These logs are written to the standard Flex Gateway logging output. Default: `false`. | No |
