...
This quick guide provides the steps to install the WAF node from the source packages on a separate Alibaba Cloud instance. By following this guide, you will create an instance from the supported operating system image and install the Wallarm WAF node on this operating system.

!!! warning "The instructions limitations"
    These instructions do not cover the configuration of load balancing and WAF node autoscaling. If setting up these components yourself, we recommend that you review the [instructions on Alibaba Cloud Elastic Compute Service (ECS)](https://www.alibabacloud.com/product/ecs).
...
To install the WAF node as the NGINX Stable module in the Alibaba Cloud instance:
...
To install the WAF node as the NGINX Plus module in the Alibaba Cloud instance:
...
To install the WAF node as the Kong module in the Alibaba Cloud instance:
