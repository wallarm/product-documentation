...
Use [search tags of filters](../../user-guides/search-and-filters/use-search.md#search-by-attack-type) to find requests from denylisted IPs: [API abuse related](../../attacks-vulns-list.md#api-abuse), `Brute force`, `Force browsing`, `BOLA`, `Malicious payload trigger` for automatically listed, `Blocked source` - for manual ones.
