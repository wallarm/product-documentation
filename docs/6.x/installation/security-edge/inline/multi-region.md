...
If your protected host is a third-level (or higher-level) domain (e.g., `api.example.com`), you need to [specify the CNAME record pointing to the Wallarm‑provided FQDN in your DNS zone](deployment.md#7-routing-traffic-to-the-edge-node). This record is returned as the **Traffic CNAME**.
