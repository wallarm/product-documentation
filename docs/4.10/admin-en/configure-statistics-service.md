...
1. Add the `status_endpoint` parameter with the new address value to the `node.yaml` file (`/opt/wallarm/etc/wallarm/node.yaml` for Docker NGINX-based image, cloud images and NGINX Node all-in-one installer - use search to locate files in other installations), e.g.:
