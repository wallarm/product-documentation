...
!!! warning "Breaking changes due to the deleted metrics"
    ...
    * `wallarm_nginx/gauge-requests` - you can use the [`wallarm_nginx/gauge-abnormal`](#number-of-requests) metric instead
...
A detailed description of the metric format is available at this [link](../monitoring/#metrics-format).
...
* **Metric:**
    ...
    * `wallarm_nginx/gauge-softmemfaults` if the limit for proton.db +lom was exceeded ([`wallarm_general_ruleset_memory_limit`](../configure-parameters-en.md#wallarm_general_ruleset_memory_limit)) 
* **Metric value:** `0`
* **Troubleshooting recommendations:**
    1. Collect data about the current state using one of the following scripts:
...
* **Metric:** `wallarm_nginx/gauge-proton_errors`
* **Metric value:** `0`
* **Troubleshooting recommendations:**
...
* **Metric:** `wallarm_nginx/gauge-db_apply_time`
* **Metric value:** no limits
...
* **Metric:** `wallarm_nginx/gauge-custom_ruleset_apply_time`

    (In Wallarm node 3.4 and lower, `wallarm_nginx/gauge-lom_apply_time`. The metric with the former name is still collected but will be deprecated soon.)
* **Metric value:** no limits
...
* **Metric value:**
    ...
    * critical if `>300`
* **Troubleshooting recommendations:**
    * Read logs from the `/var/log/wallarm/export-attacks.log` or `/opt/wallarm/var/log/wallarm/export-attacks-out.log` file [depending on a node installation method](../configure-logging.md) and analyze errors. An increased value can be caused by low network throughput from the filter node to Wallarm's API service.
