...
These instructions describe the steps to update the running Docker NGINX‑based image to the version 2.14.

!!! warning "Using credentials of already existing WAF node"
    We do not recommend to use the already existing WAF node of the previous version. Please follow these instructions to create a new WAF node of the version 2.14 and deploy it as the Docker container.
...
* Automatically creates new WAF node of the version 2.14 in the Wallarm Cloud. Created WAF node will be displayed in the Wallarm Console → **Nodes**.
...
If the deployed image of the version 2.14 operates correctly, you can delete the WAF node of the previous version in the Wallarm Console → **Nodes** section.
