...
!!! info "If the Wallarm filtering node image is already deployed in your environment"
    ...
    * If the version `3.0.x` is installed, then follow the [instructions for 3.0](/admin-en/installation-guides/envoy/envoy-docker/).
...
=== "US Cloud"
    ```bash
    docker run -d -e DEPLOY_USER='deploy@example.com' -e DEPLOY_PASSWORD='very_secret' -e ENVOY_BACKEND='example.com' -e WALLARM_API_HOST='us1.api.wallarm.com' -e TARANTOOL_MEMORY_GB=16 -p 80:80 wallarm/envoy:2.18.1-3
    ```
