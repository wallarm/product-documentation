...
2.  Make sure that the `nagios` user can receive metric values from `collectd` by executing the following test command:
    ...
    ```
    sudo -u nagios sudo /usr/bin/collectd-nagios -s /var/run/collectd-unixsock -n wallarm_nginx/gauge-attacks -H node.example.local
    ```
    
    This command allows the `nagios` user to get the value of the [`wallarm_nginx/gauge-attacks`][link-metric] metric (the number of recorded attacks) for the `node.example.local` host.
...
For example, to create a command named `check_wallarm_nginx_attacks` that will receive the `wallarm_nginx/gauge-attacks` metric for the filter node with the `node.example.local` fully qualified domain name, add the following line to the NRPE service’s configuration file:

```
command[check_wallarm_nginx_attacks]=/usr/bin/collectd-nagios -s /var/run/collectd-unixsock -n wallarm_nginx/gauge-attacks -H node.example.local
```
...
1.  Create a `/usr/local/nagios/etc/objects/nodes.cfg` file with the following contents:
    ...
    This file defines the `node.example.local` host with the `10.0.30.5` IP address and the command to check the status of the `wallarm_nginx_attacks` service, which means receiving the `wallarm_nginx/gauge-attacks` metric from the filter node (see the description of the [`check_wallarm_nginx_attacks`][anchor-header-7] command).
