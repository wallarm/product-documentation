...
!!! info "A few assumptions"
    ...
    The [`wallarm_nginx/gauge-attacks`][doc-gauge-attacks] metric, which shows the number of attacks on an application that is protected by the `node.example.local` filter node, is used as an example.
...
The query to fetch the `wallarm_nginx/gauge-attacks` metric is:
```
...
2.  Select the elements of the required metric in a sequential manner by clicking the *select metric* button for the metric’s element in the *Series* line.
    ...
    The elements of `the wallarm_nginx/gauge-attacks` metric go as follows:
...
For the `wallarm_nginx/gauge-attacks` metric, we recommend using the “Gauge” visualization:
...
After you have connected one of the data sources and configured the query and visualization for the `wallarm_nginx/gauge-attacks` metric, check the monitoring operation:
...
The Grafana dashboard now displays the `wallarm_nginx/gauge-attacks` metric values for the `node.example.local` filter node.
