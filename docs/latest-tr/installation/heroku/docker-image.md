...
# Heroku'da Wallarm Çalıştırma

Wallarm, [Heroku](https://www.heroku.com/) bulut platformunda dağıtılan web uygulamalarını ve API'leri koruyabilir. Bu rehber, Wallarm node'unu Heroku üzerinde çalıştırarak trafiği gerçek zamanlı analiz etme sürecinden geçirir.

Şu anda Wallarm tarafından Heroku için resmi bir Docker imajı bulunmamaktadır. Bu nedenle, bu rehber, [all-in-one installer][aio-docs] kullanarak kendi imajınızı oluşturma ve çalıştırma adımlarını açıklar.

## Gereksinimler

* Ev sahibi sisteminizde kurulu [Docker](https://docs.docker.com/engine/install/)
* Heroku Wallarm Docker imajını push etmek için Docker hesabı
* Ev sahibi sisteminizde kurulu [Heroku CLI](https://devcenter.heroku.com/articles/heroku-cli)
* Heroku web dynolarında çalışan uygulamalar veya API'ler
* [US Cloud](https://us1.my.wallarm.com/) veya [EU Cloud](https://my.wallarm.com/) içindeki Wallarm Console'a yönetici erişimi
* Tümleşik Wallarm kurulumunu indirmek için `https://meganode.wallarm.com` erişimi
...
* Saldırı tespit kuralları güncellemelerini ve [API spesifikasyonlarını][api-spec-enforcement-docs] indirmek; ayrıca [allowlisted, denylisted veya graylisted][ip-lists-docs] ülkeler, bölgeler veya veri merkezleri için kesin IP'leri almak amacıyla aşağıdaki IP adreslerine erişim:

    --8<-- "../include/wallarm-cloud-ips.md"

## Adım 1: Wallarm Docker Yapılandırmasının Hazırlanması

Wallarm'ın Docker imajını Heroku üzerinde dağıtmak için, imaj oluşturma süreci için gerekli yapılandırma dosyalarını oluşturun. Aşağıdaki adımları takip edin:

1. Lokal sisteminizde, yalnızca Wallarm Docker yapılandırmaları için özel bir dizin oluşturun ve bu dizine geçin.
1. NGINX ve [Wallarm yapılandırmalarını][waf-directives-instr] içeren bir `nginx.conf` dosyası oluşturun. Docker imajı, [NGINX uyumlu all-in-one installer][aio-docs] tabanlı olacağından, NGINX’in doğru şekilde yapılandırıldığından emin olun.

    Aşağıda, izleme modunda Wallarm node'unu çalıştıran temel bir yapılandırmayı içeren şablon yer almaktadır:
    ...
    ```
1. Aşağıdaki `entrypoint.sh` dosyasını, Wallarm Docker imajı için gerekli yönergeleri içerecek şekilde oluşturun:
...
1. `entrypoint.sh` dosyasının izinlerini `-rwxr-xr-x` olarak ayarlamak için aşağıdaki komutu çalıştırın:

    ```
    chmod 755 entrypoint.sh
    ```
1. Wallarm tarafından engellenecek istekler için iyi yapılandırılmış bir sayfa gösteren bir `403.html` dosyası tasarlayın. Aşağıdakileri kopyalayabilirsiniz:
...
1. Wallarm'ın Docker imajı için yapı sürecini tanımlayan bir `Dockerfile` oluşturun:
...
1. Uygulamanızın çalışma zamanı için gerekli bağımlılıkların kurulmasını içerecek şekilde bir `Dockerfile` oluşturun. Örneğin, bir Node.js uygulaması için aşağıdaki şablonu kullanın:
    ...
    CMD ["npm", "run", "start"]
    ```
1. Aşağıdaki içeriğe sahip bir `heroku.yml` yapılandırma dosyası oluşturun:
...
1. Uygulamanızı, Wallarm yapılandırılmamışsa `$PORT` üzerinde, yapılandırılmışsa `/tmp/nginx.socket` üzerinde dinleyecek şekilde uyarlayın. Örneğin, yapılandırmanız aşağıdaki gibi olabilir:
    ...
    ```
1. Wallarm node örneğini Wallarm Cloud ile ilişkilendirmek için [uygun tipte][node-token-types] bir filtreleme node token'ı oluşturun:

    === "API token"
        1. Wallarm Console → **Settings** → **API tokens** bölümüne, [US Cloud](https://us1.my.wallarm.com/settings/api-tokens) veya [EU Cloud](https://my.wallarm.com/settings/api-tokens) üzerinden gidin.
        1. `Deploy` kaynağı rolüne sahip bir API token'ı bulun veya oluşturun.
        1. Bu token'ı kopyalayın.
        1. Wallarm node'unu ekleyeceğiniz node grubunun adını aşağıdaki ortam değişkeninde belirtin:

        ```
        heroku config:set WALLARM_LABELS=group=<NODE_GROUP_NAME>
        ```
    === "Node token"
        1. Wallarm Console → **Nodes** bölümüne, [US Cloud](https://us1.my.wallarm.com/nodes) veya [EU Cloud](https://my.wallarm.com/nodes) üzerinden gidin.
        1. **Wallarm node** türünde bir filtreleme node oluşturun ve oluşturulan token'ı kopyalayın.
1. Node'un Cloud ile bağlantısını sağlamak için ilgili ortam değişkenlerini ayarlayın:
    ...
    === "EU Cloud"
        ...
        heroku config:set WALLARM_API_TOKEN=<NODE_TOKEN>
        ```
1. Uygulamanızı yeniden başlatmak ve Wallarm node'unu dağıtmak için uygulamanızı push edin:
...
Dağıtımın çalıştığından emin olmak için, [Path Traversal][ptrav-attack-docs] açığı kullanılarak bir test saldırısı başlatın:

```
curl http://<HEROKU_APP_DOMAIN>/etc/passwd
```

Node, varsayılan olarak **monitoring** [filtration mode][filtration-mode-docs] modunda çalıştığından, Wallarm node saldırıyı engellemez ancak kaydeder. Saldırının kaydedildiğini kontrol etmek için Wallarm Console → **Attacks** bölümüne gidin:
...
Dağıtım sırasında yardıma ihtiyaç duyarsanız, [Wallarm support team](mailto:support@wallarm.com) ile iletişime geçin.