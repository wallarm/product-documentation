...
2.  `nagios` kullanıcısının `collectd`'den metrik değerlerini alabileceğinden emin olmak için aşağıdaki test komutunu çalıştırın:
    ...
    Bu komut, `node.example.local` ana makinesi için [`curl_json-wallarm_nginx/gauge-abnormal`][link-metric] metriğinin (işlenen istek sayısı) değerini almayı sağlar.
...
Örneğin, `node.example.local` tam nitelikli alan adına sahip filtre düğümü için `curl_json-wallarm_nginx/gauge-abnormal` metriğini alacak olan `check_wallarm_nginx_abnormal` adında bir komut oluşturmak için, NRPE servisi yapılandırma dosyasına aşağıdaki satırı ekleyin:
...
1.  `/usr/local/nagios/etc/objects/nodes.cfg` dosyasını aşağıdaki içerikle oluşturun:
    ...
    Bu dosya, `10.0.30.5` IP adresiyle `node.example.local` ana makinesini ve filtre düğümünden `curl_json-wallarm_nginx/gauge-abnormal` metriğinin alınarak `wallarm_nginx_abnormal` servisi durumunun kontrol edileceğini tanımlar (bkz. [`check_wallarm_nginx_abnormal`][anchor-header-7] komutunun açıklaması).
