...
* **Metrik:**
    * Sisteminizde limit aşıldığında `wallarm_nginx/gauge-memfaults`
    * Proton.db +lom limiti aşıldığında `wallarm_nginx/gauge-softmemfaults` ([`wallarm_general_ruleset_memory_limit`](../configure-parameters-en.md#wallarm_general_ruleset_memory_limit)) 
* **Metrik değeri:** `0`
* **Sorun Giderme Önerileri:**
    1. `/opt/wallarm/collect-info.sh` betiğini kullanarak mevcut durum ile ilgili veri toplayın.
...
* **Metrik:** `wallarm_nginx/gauge-custom_ruleset_id`
    ...
    (Wallarm node 3.4 ve öncesinde, `wallarm_nginx/gauge-lom_id`. Eski isimli metrik hâlâ toplanıyor ancak yakın gelecekte kullanımdan kaldırılacak.)
* **Metrik değeri:** herhangi bir limit yok
...
* **Metrik:** `wallarm_nginx/gauge-custom_ruleset_apply_time`
    ...
    (Wallarm node 3.4 ve öncesinde, `wallarm_nginx/gauge-lom_apply_time`. Eski isimli metrik hâlâ toplanıyor ancak yakın gelecekte kullanımdan kaldırılacak.)
* **Metrik değeri:** herhangi bir limit yok
