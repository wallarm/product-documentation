...
- [API Security Testing Overview](https://docs.wallarm.com/5.x/about-wallarm/detecting-vulnerabilities.md): What a security issue is and the two methods Wallarm uses to detect them — passive detection and AASM (also: detection methods, security testing).
...
- [Wallarm Scanner IP Addresses](https://docs.wallarm.com/5.x/admin-en/scanner-addresses.md): The fixed list of US/EU Cloud IPs Wallarm uses for active scans (allowlist these on origin).
...
- [Docs MCP Server](https://docs.wallarm.com/5.x/reference/docs-mcp-server.md): Connect an MCP client to this documentation. Read-only MCP server over streamable HTTP at `https://mcp-docs.wallarm.com/mcp` (server card: https://docs.wallarm.com/.well-known/mcp/server-card.json); exposes `search_pages`, `get_page`, `list_pages`, `list_versions` so AI clients can search and read Wallarm docs with citations.
