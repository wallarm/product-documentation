...
2. Run the plugin:
    ...
    ```bash
    helm diff upgrade <RELEASE_NAME> -n wallarm-sidecar wallarm/wallarm-sidecar --version 5.3.12 -f <PATH_TO_VALUES>
    ```
...
``` bash
helm install --version 5.3.12 <RELEASE_NAME> wallarm/wallarm-sidecar --wait -n wallarm-sidecar -f <PATH_TO_VALUES>
```
...
``` bash
helm upgrade <RELEASE_NAME> -n <NAMESPACE> wallarm/wallarm-sidecar --version 5.3.12 -f <PATH_TO_VALUES>
```
...
1. Make sure the version of the Helm chart was upgraded:
    ...
    The chart version should correspond to `wallarm-sidecar-5.3.12`.
