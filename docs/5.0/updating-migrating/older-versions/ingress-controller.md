...
--8<-- "../include/waf/installation/requirements-nginx-ingress-controller-5.x.md"
...
2. Run the plugin:
    ...
    * `<PATH_TO_VALUES>`: the path to the `values.yaml` file defining the [Ingress controller 5.0 settings](#step-4-update-the-valuesyaml-configuration)
...
2. Deploy the Ingress controller 5.0:
    ...
    ```bash
    helm install <RELEASE_NAME> -n <NAMESPACE> wallarm/wallarm-ingress --version 5.3.18 -f <PATH_TO_VALUES>
    ```
...
1. Check that the version of the Helm chart was updated:
    ...
    The chart version should correspond to `wallarm-ingress-5.3.16`.
