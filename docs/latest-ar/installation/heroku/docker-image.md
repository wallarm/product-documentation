...
يمكن لـ Wallarm حماية التطبيقات الويب وواجهات برمجة التطبيقات المنشورة على منصة السحابة [Heroku](https://www.heroku.com/). تعرض هذا الدليل لك العملية خطوة بخطوة لتشغيل نقطة الـ Wallarm على Heroku لتحليل حركة المرور في الوقت الفعلي.

حتى الآن، ليست هناك صورة Docker رسمية لـ Heroku من Wallarm. لذا، يشرح هذا الدليل كيفية إنشاء صورة خاصة بك وتشغيلها باستخدام المثبت [كل في واحد][aio-docs].

## المتطلبات

* [Docker](https://docs.docker.com/engine/install/) مثبت على نظامك المضيف
* حساب Docker لدفع صورة Docker لـ Wallarm في Heroku
* [هيركو CLI](https://devcenter.heroku.com/articles/heroku-cli) مثبت على نظامك المضيف
* التطبيقات أو واجهات برمجة التطبيقات تعمل على الويب dynos في Heroku
* صلاحيات المسؤول الخاصة بكتابة الأوامر في وحدة التحكم Wallarm على [السحابة الأمريكية](https://us1.my.wallarm.com/) أو [السحابة الأوروبية](https://my.wallarm.com/)
* الوصول إلى `https://meganode.wallarm.com` لتحميل المثبت الخاص بـ Wallarm المتكامل
...
* الوصول إلى عناوين IP لـ Google Cloud Storage المدرجة ضمن [الرابط](https://www.gstatic.com/ipranges/goog.json). عندما تسمح بالدخول، ترفض الدخول، أو تضع قائمة رمادية [للسماح بالدخول، أو لرفضه، أو لقائمة رمادية][ip-lists-docs] لتضم دول بأكملها، مناطق، أو مراكز بيانات بدلاً من عناوين IP فردية، تقوم نقطة Wallarm باسترداد العناوين IP الدقيقة المتعلقة بالإدخالات في قوائم IP من قاعدة البيانات المجمعة المستضافة على Google Storage.

## الخطوة 1: إعداد تكوين Docker لـ Wallarm

لنشر صورة Docker لـ Wallarm على Heroku، قم أولاً بإعداد الملفات التكوينية اللازمة لعملية بناء الصورة. اتبع الخطوات التالية:

1. على نظامك المحلي، أنشئ دليل خاص بتكوينات Docker لـ Wallarm واستمر فيه.
1. أنشئ ملفًا باسم `nginx.conf` يتضمن تفاصيل التكوين لـ NGINX. نظرًا لأن الصورة Docker ستعتمد على [المثبت المتكامل المتوافق مع NGINX][aio-docs]، من الأهمية تكوين NGINX وفقًا لذلك.
...
1. قم بصياغة ملف `default.conf` بتكوين الـ Wallarm. استخدم القواعد الموجهة لـ NGINX في Wallarm[waf-directives-instr] لتخصيص نقطة Wallarm وفقا لمتطلباتك الخاصة.

    ها هو القالب الذي يحتوي على التكوين الأساسي الذي يعمل على تشغيل نقطة Wallarm في وضع المراقبة:
    ...
    server {
      ...
      location / {
        ...
        # Applications Heroku are always behind a load balancer, which is why we consider all internet addresses trusted
...
1. قم بإنشاء الملف القادم `entrypoint.sh` الذي يتضمن التوجيهات لصورة Docker لـ Wallarm:
...
1. اضبط أذونات الملف `entrypoint.sh` ل -rwxr-xr-x عن طريق تنفيذ الأمر التالي:

    ```
    chmod 755 entrypoint.sh
    ```
1. قم بتصميم ملف `403.html` الذي يعرض صفحة مرتبة بشكل جيد للطلبات التي ستمنعها Wallarm. يمكنك نسخ ما يلي:
...
## الخطوة 2: إنشاء صورة Docker لـ Wallarm لـ Heroku
...
## الخطوة 3: تشغيل الصورة المبنية على Heroku

لنشر الصورة على Heroku:

1. ابحث عن الجذر الخاص بدليل تطبيقك لتنفيذ العمليات التالية.
1. قم ببناء `Dockerfile` التي ستتضمن تثبيت التبعيات اللازمة المحددة لوقت تشغيل تطبيقاتك. في التطبيقات Node.js ، استخدم القالب التالي:
...
1. قم بتكييف تطبيقك للاستماع على `/tmp/nginx.socket` بدلاً من `$PORT` لأن `$PORT` مستخدمة بواسطة NGINX. على سبيل المثال، قد تبدو التهيئة كما يلي:
    ...
    ```
1. قم بتوليد رمز نقطة التصفية الذي يناسب [النوع المناسب][node-token-types] لربط مثيل نقطة Wallarm بـ سحابة Wallarm:
    ...
    === "رمز API"
        ...
        1. ابحث أو أنشأ رمز API بالدور المصدر `Deploy`.
        1. انسخ هذا الرمز.
        1. حدد اسم مجموعة النقاط لإضافة نقطة Wallarm إليه في المتغير البيئي التالي:

        ```
        heroku config:set WALLARM_LABELS=group=<NODE_GROUP_NAME>
        ```
    === "رمز النقطة"
        1. افتح وحدة التحكم Wallarm → **النقاط** في إما [السحابة الأمريكية](https://us1.my.wallarm.com/nodes) أو [السحابة الأوروبية](https://my.wallarm.com/nodes).
        1. أنشئ نقطة تصفية بنوع "نقطة الـ Wallarm" وانسخ الرمز المولد.
1. أنشئ التحكم لربط النقطة مع السحابة ضمن المتغيرات ذات الصلة:
    ...
    === "السحابة الأوروبية"
        ...
        heroku config:set WALLARM_API_TOKEN=<NODE_TOKEN>
        ```
1. ادفع التطبيق الخاص بك لتحفيز إعادة التشغيل، مما يتيح نشر نقطة Wallarm:
...
## الخطوة 4: اختبر التنصيب

لتأكيد أن التنصيب يعمل، شغل هجوم اختبار باستخدام تقنية اختراق المسار [Path Traversal][ptrav-attack-docs]:

```
curl http://<HEROKU_APP_DOMAIN>/etc/passwd
```

بما أن النقطة تعمل في وضع [الترشيح][filtration-mode-docs] **المراقبة** بشكل افتراضي، فلا تقوم نقطة Wallarm بمنع الهجوم ولكنها تقوم بتسجيله. للتحقق من أنه تم تسجيل الهجوم، انتقل إلى وحدة التحكم Wallarm → **الهجمات**:

![الهجمات في الواجهة][attacks-in-ui-image]

## تصحيح الأخطاء

إذا واجهت أي صعوبات مع صورة Docker الأساسية لـ Wallarm، فحلل سجلات Heroku للرسائل الخطأ المحتملة:

```
heroku logs --tail
```

إذا كنت بحاجة إلى مساعدة أثناء التنصيب، اتصل ب[fريق الدعم في Wallarm](mailto:support@wallarm.com).