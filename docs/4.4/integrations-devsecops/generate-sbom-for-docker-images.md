...
```bash
syft wallarm/ingress-controller:4.4.5-1
```
...
```bash
syft wallarm/ingress-controller:4.4.5-1 --output spdx-json >> syft_json_sbom.spdx
syft wallarm/ingress-controller:4.4.5-1 --output cyclonedx-json >> cyclonedx_json_sbom.cyclonedx
```
