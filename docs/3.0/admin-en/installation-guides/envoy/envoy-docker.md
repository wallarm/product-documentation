...
!!! info "If the Wallarm filtering node image is already deployed in your environment"
    ...
    * If the version `3.2.x` is installed, then follow the [instructions for 3.2](/3.2/admin-en/installation-guides/envoy/envoy-docker/) or [update the packages to the latest version](/updating-migrating/docker-container/) in all deployments.
...
=== "EU Cloud"
    ```bash
    docker run -d -e DEPLOY_USER='deploy@example.com' -e DEPLOY_PASSWORD='very_secret' -e ENVOY_BACKEND='example.com' -p 80:80 wallarm/envoy:3.0.0-1
    ```
=== "US Cloud"
    ```bash
    docker run -d -e DEPLOY_USER='deploy@example.com' -e DEPLOY_PASSWORD='very_secret' -e ENVOY_BACKEND='example.com' -e WALLARM_API_HOST='us1.api.wallarm.com' -p 80:80 wallarm/envoy:3.0.0-1
    ```
