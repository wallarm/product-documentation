...
*   `requests`: the number of requests that have been processed by the filter node except for those originated from [blacklisted](../user-guides/ip-lists/blacklist.md) IPs.
*   `attacks`: the number of recorded attacks.
*   `blocked`: the number of blocked requests except for those originated from [blacklisted](../user-guides/ip-lists/blacklist.md) IPs.
