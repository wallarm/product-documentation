...
[node-token-types]:                      ../../../user-guides/nodes/nodes.md#api-and-node-tokens-for-node-creation#api-and-node-tokens-for-node-creation
...
1. Install the Wallarm packages:
    ...
    ``` bash
    helm install --version 4.8.9 <RELEASE_NAME> wallarm/wallarm-ingress -n <KUBERNETES_NAMESPACE> -f <PATH_TO_VALUES>
    ```
