...
data:
  default: |
    geo $remote_addr $wallarm_mode_real {
      ...
      # blockは全てのリクエストを処理し悪意あるリクエストをブロックします
      default <WALLARM_MODE>;
      # 米国のクラウドスキャナーのIPアドレスとルール
    ...
    server {
        ...
        location / {
                # 以下の<APP_CONTAINER_PORT>を
                # コンテナが受信リクエストを受け付けるポート番号に置き換えてください、
                # 値はmain app containerの定義におけるports.containerPortと同一である必要があります
