...
## 2. Wallarmパッケージを含むNGINXをインストールする

以下のパッケージがインストールされます：

* `nginx` はNGINX用
* `libnginx-mod-http-wallarm` または `nginx-mod-http-wallarm` はNGINX-Wallarmモジュール用
* `wallarm-node` は[postanalytics][install-postanalytics-docs]モジュール、Tarantoolデータベース、および追加のNGINX-Wallarmパッケージ用
